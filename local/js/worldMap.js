...
(function() {
  ...
  PopBig = (function(_super) {
    ...
    function PopBig(tpl, data, woldMap, name) {
      var _this = this;

      PopBig.__super__.constructor.call(this, tpl);
      this.game = woldMap.game;
      this.woldMap = woldMap;
  ...
  MapPoint = (function(_super) {
    ...
    function MapPoint(tpl, data, menu, woldMap, name) {
      var _this = this;
  ...
  window.WorldMap = (function(_super) {
    ...
    function WorldMap(game) {
      ...
      _ref = ["home", "guild", "magicItemShop", "equipmentShop", "forest", "snowmountain"];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        name = _ref[_i];
        data = this.db.areas.get(name);
        imgName = data.summaryImg;
        img = window.Res.imgs[imgName];
        newItem = new MapPoint(this.menu.UI['map-point-tpl'].innerHTML, data, this.menu, this, name);
        newItem.appendTo(this.menu.UI['map-summary-holder']);
        this.areaItems.push(newItem);
      }
      if (this.areaItems.length > 4) {
        this.menu.UI['map-summary-holder'].J.addClass("two-lines");
      }
    }
